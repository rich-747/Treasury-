...
// ══════════════════════════════════════════════════════════════════
//  MAIN TREASURY APP  (unchanged from your version)
// ══════════════════════════════════════════════════════════════════
function TreasuryApp({group,userProfile,allGroups=[],onSwitchGroup,onBack,onUpdateProfile,dark,onToggleDark}){
  ...
  useEffect(()=>{const handler=e=>{if(headerRef.current&&!headerRef.current.contains(e.target))setSwitcherOpen(false);};document.addEventListener("mousedown",handler);return()=>document.removeEventListener("mousedown",handler);},[]);
  ...
  const markPaid=async mid=>{if(paidIds.includes(mid)){showT("Already marked paid!","info");return;}await upGroup({contributions:[...(gData.contributions||[]),{id:gData.nextId,memberId:mid,month:thisMonth,amount:gData.monthlyAmount||200,date:new Date().toISOString(),markedBy:userProfile.uid}],nextId:(gData.nextId||100)+1});showT("Payment marked ✓");};
  const notifyMember=async m=>{await upGroup({notifications:[...(gData.notifications||[]),{id:(gData.nextId||100)+1,type:"payment_reminder",toUid:m.uid,fromName:userProfile.name,message:`${userProfile.avatar} ${userProfile.name} reminded you to pay ₹${gData.monthlyAmount||200} for ${new Date().toLocaleString("default",{month:"long"})}!`,createdAt:new Date().toISOString(),read:false}],nextId:(gData.nextId||100)+2});showT(`Notification sent to ${m.name}! 🔔`);};
  const addVote=async f=>{await upGroup({votes:[...(gData.votes||[]),{id:gData.nextId,title:f.title,amount:Number(f.amount),category:f.category,description:f.description||"",requestedBy:userProfile.uid,requestedByName:userProfile.name,requestedByAvatar:userProfile.avatar,createdAt:new Date().toISOString(),approvals:[],rejections:[],status:"pending"}],nextId:(gData.nextId||100)+1});showT("Expense sent for vote! 🗳️");closeModal();};
  ...
  const nominateAdmin=async nomineeUid=>{if(adminVotes.find(v=>v.nomineeUid===nomineeUid&&v.status==="pending")){showT("Already nominated!","info");return;}if(currentAdmins.length>=maxAdmins){showT(`Max ${maxAdmins} admins allowed!`,"error");return;}await upGroup({adminVotes:[...adminVotes,{id:gData.nextId,nomineeUid,nominatedBy:userProfile.uid,nominatedByName:userProfile.name,createdAt:new Date().toISOString(),approvals:[userProfile.uid],status:"pending"}],nextId:(gData.nextId||100)+1});showT("Nomination sent! 👑");};
  ...
  const addEmergency=async f=>{await upGroup({emergencyRequests:[...(gData.emergencyRequests||[]),{id:gData.nextId,memberId:userProfile.uid,memberName:userProfile.name,memberAvatar:userProfile.avatar,amount:Number(f.amount),reason:f.reason,details:f.details||"",createdAt:new Date().toISOString(),approvals:[],rejections:[],status:"pending"}],nextId:(gData.nextId||100)+1});showT("Emergency sent! 🆘");closeModal();};
